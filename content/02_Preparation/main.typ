...
As a consequence of @lean:li:compiler we have a tension between the logical and computation sides of a proofs/programs.
...
we need some inductive datatype (@sec:ind),
...
One crucial polynomial not shown here is composition. The code of composition is not very insightful.
...
== Recursion and Inductive datatype<sec:ind>

#set raw(lang: "ocaml")

#definition[
  An inductive datatype, is a type with a collection of constructors,
  and an eliminator consuming the tree and produces some data.
  Objects of inductive datatypes are *well-founded* trees of data.
]<rec:ind:def>

Recalling algebraic datatypes from languages such as OCaml, Haskell and Rust.
...
If we restrict these datatypes to well-founded trees,
these are exactly inductives#footnote[
...
We also have two operations `mk : F (μ F) → F` and `dest : μ F → F (μ F)`.
These form an equivalence (#cite(<cite:lambek>, form: "prose") Corollary 2.4),
giving us the result that the cofix-point of `F`, written as `μ F`, is equivalent to `F (μ F)`.
In practice this means we can unfold `μ F`.
...
#definition[
  A coinductive datatype is a type with a collection of constructors,
  and a generating function.
  Objects of a coinductive datatypes are potentially infinite trees of data.
]
...
possibly infinitely deep trees in which each layer is an unfolding of some polynomial `F`.
...
The state-machine encoding (@sec:m:sme) and Progressive Approximation (@sec:m:pa) (as seen in #MATHLIB).
...
We do not need to worry about implementing and proving this as #MATHLIB @cite:keizer already provides an implementation of this.
...
#definition[
  ...
  `return >>= a = a`, `f >>= return v = f v` and `x >>= f >>= g = x >>= λx ↦ f x >>= g`.
]
...
as in `simp`s that do not close a goal,
...
+ The AltRepr Type COULD be zero-cost. <rq:abi:zc>
], [
// +
])
...
+ Free Monad WONT be hetrogenous @cite:coco.
])
