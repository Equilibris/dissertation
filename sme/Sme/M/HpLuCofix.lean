...
def corecF {α : TypeVec n} {β : Type u} (g : β → F (α.append1 β)) : β → M q.P α :=
...
def IsPrecongr {α : TypeVec n} (r : M q.P α → M q.P α → Prop) : Prop :=
  ∀ ⦃x y⦄,
    r x y →
      abs (appendFun id (Quot.mk r) <$$> M.dest x) =
        abs (appendFun id (Quot.mk r) <$$> M.dest y)

/-- Equivalence relation on M-types representing a value of type `Cofix F` -/
def Mcongr {α : TypeVec n} (x y : M q.P α) : Prop :=
...
def mRepr {α : TypeVec n} : M q.P α → M q.P α :=
  corecF (abs ∘ M.dest)

/-- the map function for the functor `Cofix F` -/
def Cofix.map {α β : TypeVec n} (g : α ⟹ β) : Cofix F α → Cofix F β :=
  Quot.lift (fun x : M q.P α => Quot.mk Mcongr (g <$$> x))
    (by
      rintro aa₁ aa₂ ⟨r, pr, ra₁a₂⟩; apply Quot.sound
      let r' b₁ b₂ := ∃ a₁ a₂ : M q.P α, r a₁ a₂ ∧ b₁ = g <$$> a₁ ∧ b₂ = g <$$> a₂
      use r'; constructor
      · show IsPrecongr r'
        rintro b₁ b₂ ⟨a₁, a₂, ra₁a₂, b₁eq, b₂eq⟩
        let u : Quot r → Quot r' :=
          Quot.lift (fun x : M q.P α => Quot.mk r' (g <$$> x))
            (by
              intro a₁ a₂ ra₁a₂
              apply Quot.sound
              exact ⟨a₁, a₂, ra₁a₂, rfl, rfl⟩)
        have hu : (Quot.mk r' ∘ fun x : M q.P α => g <$$> x) = u ∘ Quot.mk r := by
          ext x
          rfl
...
def Cofix.dest {α : TypeVec n} : Cofix F α → F (α.append1 (Cofix F α)) :=
  Quot.lift (fun x => appendFun id (Quot.mk Mcongr) <$$> abs (M.dest x))
    (by
      rintro x y ⟨r, pr, rxy⟩
      dsimp
...
def Cofix.abs {α} : M q.P α → Cofix F α :=
  Quot.mk _

/-- Representation function for `Cofix F α` -/
def Cofix.repr {α} : Cofix F α → M q.P α :=
  M.corec' <| q.repr ∘ Cofix.dest
...
private theorem Cofix.bisim_aux {α : TypeVec n} (r : Cofix F α → Cofix F α → Prop) (h' : ∀ x, r x x)
  ...
  have : IsPrecongr r' := by
    intro a b r'ab
    have h₀ :
      appendFun id (Quot.mk r ∘ Quot.mk Mcongr) <$$> MvQPF.abs (M.dest a) =
        appendFun id (Quot.mk r ∘ Quot.mk Mcongr) <$$> MvQPF.abs (M.dest b) := by
      rw [appendFun_comp_id, LawfulMvFunctor.comp_map, LawfulMvFunctor.comp_map]; exact h _ _ r'ab
    have h₁ : ∀ u v : M q.P α, Mcongr u v → Quot.mk r' u = Quot.mk r' v := by
      intro u v cuv
      apply Quot.sound
      dsimp [r', hr']
      rw [Quot.sound cuv]
...
theorem Cofix.abs_repr {α} (x : Cofix F α) : Quot.mk _ (Cofix.repr x) = x := by
  ...
  simp only [repr, M.dest_corec', Function.comp, abs_map, MvQPF.abs_repr]
