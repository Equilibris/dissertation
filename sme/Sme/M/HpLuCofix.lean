...
def corecF {α : TypeVec n} {β : Type u} (g : β → F (α.append1 β)) : β → HpLuM q.P α :=
  .corec' fun x => repr (g x)

theorem corecF_eq {α : TypeVec n} {β : Type u} (g : β → F (α.append1 β)) (x : β) :
    (corecF g x).dest = appendFun id (corecF g) <$$> repr (g x) := by
  rw [corecF, HpLuM.dest_corec']

/-- Characterization of desirable equivalence relations on M-types -/
def IsPrecongr {α : TypeVec n} (r : HpLuM q.P α → HpLuM q.P α → Prop) : Prop :=
  ∀ ⦃x y⦄,
    r x y →
      abs (appendFun id (Quot.mk r) <$$> HpLuM.dest x) =
        abs (appendFun id (Quot.mk r) <$$> HpLuM.dest y)

/-- Equivalence relation on M-types representing a value of type `Cofix F` -/
def Mcongr {α : TypeVec n} (x y : HpLuM q.P α) : Prop :=
...
def mRepr {α : TypeVec n} : HpLuM q.P α → HpLuM q.P α :=
  corecF (abs ∘ HpLuM.dest)

/-- the map function for the functor `Cofix F` -/
def Cofix.map {α β : TypeVec n} (g : α ⟹ β) : Cofix F α → Cofix F β :=
  Quot.lift (fun x : HpLuM q.P α => Quot.mk Mcongr (g <$$> x))
    (by
      rintro aa₁ aa₂ ⟨r, pr, ra₁a₂⟩; apply Quot.sound
      let r' b₁ b₂ := ∃ a₁ a₂ : HpLuM q.P α, r a₁ a₂ ∧ b₁ = g <$$> a₁ ∧ b₂ = g <$$> a₂
      use r'; constructor
      · show IsPrecongr r'
        rintro b₁ b₂ ⟨a₁, a₂, ra₁a₂, b₁eq, b₂eq⟩
        let u : Quot r → Quot r' :=
          Quot.lift (fun x : HpLuM q.P α => Quot.mk r' (g <$$> x))
            (by
              intro a₁ a₂ ra₁a₂
              apply Quot.sound
              exact ⟨a₁, a₂, ra₁a₂, rfl, rfl⟩)
        have hu : (Quot.mk r' ∘ fun x : HpLuM q.P α => g <$$> x) = u ∘ Quot.mk r := by
          ext x
          rfl
        rw [b₁eq, b₂eq, HpLuM.dest_map, HpLuM.dest_map, ← q.P.comp_map, ← q.P.comp_map]
...
def Cofix.dest {α : TypeVec n} : Cofix F α → F (α.append1 (Cofix F α)) :=
  Quot.lift (fun x => appendFun id (Quot.mk Mcongr) <$$> abs (HpLuM.dest x))
    (by
      rintro x y ⟨r, pr, rxy⟩
      dsimp
      ...
      conv =>
        lhs
        rw [appendFun_comp_id, LawfulMvFunctor.comp_map, ← abs_map, pr rxy, abs_map, 
          ← LawfulMvFunctor.comp_map, ← appendFun_comp_id])

/-- Abstraction function for `cofix F α` -/
def Cofix.abs {α} : HpLuM q.P α → Cofix F α :=
  Quot.mk _

/-- Representation function for `Cofix F α` -/
def Cofix.repr {α} : Cofix F α → HpLuM q.P α :=
  HpLuM.corec' <| q.repr ∘ Cofix.dest
...
theorem Cofix.dest_corec {α : TypeVec n} {β : Type u} (g : β → F (α.append1 β)) (x : β) :
  ...
  dsimp
  rw [corecF_eq, abs_map, abs_repr, ← LawfulMvFunctor.comp_map, ← appendFun_comp]; rfl
...
private theorem Cofix.bisim_aux {α : TypeVec n} (r : Cofix F α → Cofix F α → Prop) (h' : ∀ x, r x x)
    ...
    ∀ x y, r x y → x = y := by
  intro x
  rcases x; clear x; rename_i x
  intro y
  rcases y; clear y; rename_i y
  intro rxy
  apply Quot.sound
  ...
  have : IsPrecongr r' := by
    intro a b r'ab
    have h₀ :
      appendFun id (Quot.mk r ∘ Quot.mk Mcongr) <$$> MvQPF.abs (HpLuM.dest a) =
        appendFun id (Quot.mk r ∘ Quot.mk Mcongr) <$$> MvQPF.abs (HpLuM.dest b) := by
      rw [appendFun_comp_id, LawfulMvFunctor.comp_map, LawfulMvFunctor.comp_map]; exact h _ _ r'ab
    have h₁ : ∀ u v : HpLuM q.P α, Mcongr u v → Quot.mk r' u = Quot.mk r' v := by
      intro u v cuv
      apply Quot.sound
      dsimp [r', hr']
      rw [Quot.sound cuv]
...
theorem Cofix.bisim' {α : TypeVec n} {β : Type*} (Q : β → Prop) (u v : β → Cofix F α)
  ...
  Cofix.bisim R
    (fun x y ⟨x', Qx', xeq, yeq⟩ => by
      ...
      apply (liftR_iff _ x.dest y.dest).mpr
      refine
...
theorem Cofix.mk_dest {α : TypeVec n} (x : Cofix F α) : Cofix.mk (Cofix.dest x) = x := by
  ...
  conv =>
    ...
    rw [Cofix.mk]
    rw [Cofix.dest_corec]
  rw [← LawfulMvFunctor.comp_map, ← appendFun_comp, id_comp]
  rw [← LawfulMvFunctor.comp_map, ← appendFun_comp, id_comp, ← Cofix.mk]
  congr 1
  apply congrArg
  funext x
  apply Quot.sound
...
theorem Cofix.dest_mk {α : TypeVec n} (x : F (α.append1 <| Cofix F α)) :
  ...
  rw [Cofix.mk, Cofix.dest_corec, ← LawfulMvFunctor.comp_map, ← Cofix.mk, ← appendFun_comp, this, 
    id_comp, appendFun_id_id, MvFunctor.id_map]
...
theorem Cofix.abs_repr {α} (x : Cofix F α) : Quot.mk _ (Cofix.repr x) = x := by
  ...
  simp only [repr, HpLuM.dest_corec', Function.comp, abs_map, MvQPF.abs_repr]
    ...
    withMainContext do
      ...
      withMainContext do
        ...
        withMainContext do
          ...
          evalTactic <| ← `(tactic|
            refine Sme.Cofix.bisim₂ $sR ?_ _ _ ⟨_, rfl, rfl⟩;
