...
theorem transform_dest
  ...
  simp only [dest_corec, Function.comp_apply, MvPFunctor.M.dest_corecU]
...
def dest : M P α → P (α ::: M P α) :=
  AltRepr.rec
    ((TypeVec.id ::: AltRepr.mkA) <$$> MvPFunctor.M.dest P ·)
...
def IsBisim (R : M.{u} P α → M.{u} P α → Prop) : Prop :=
    ∀ s t, R s t → MvFunctor.LiftR (TypeVec.RelLast _ R) s.dest t.dest

def Bisim : M.{u} P α → M.{u} P α → Prop := (∃ R, IsBisim R ∧ R · ·)

theorem bisim' {a b : M.{u} P α} : Bisim a b → a = b := by
  intro ⟨r, ris, rab⟩
  apply a.extB
  intro a b rfl rfl
...
theorem bisim
    ...
    (x : R a b)
    (h : ∀ s t, R s t → ∃ x, ∃ (y z : P.B x ⟹ α ::: M P α),
...
@[specialize n P α β]
def map {α β : TypeVec.{u} n} (m : α ⟹ β) : M P α → M P β :=
  corec' ((m ::: id) <$$> ·.dest)

instance : MvFunctor (M P) where map := map

theorem id_map (x : M P α) : TypeVec.id <$$> x = x := by
...
instance : LawfulMvFunctor (M P) where
  id_map := id_map
  comp_map := comp_map
...
def mkE : F.app (α ::: M P α) → M P α :=
  M.mk ∘ EquivP.equiv.symm

-- TODO: Get this right
instance : CoeHead (F.app (α ::: M P α)) (M P α) where
  coe := mkE

def destE : M P α → F.app (α ::: M P α) :=
  EquivP.equiv ∘ M.dest
...
theorem destE_mkE
  ...
  dsimp [destE, mkE]
  rw [Equiv.symm_apply_apply, M.dest_mk]
...
theorem mkE_destE
    ...
    : destE (mkE v) = v := by
  dsimp [destE, mkE]
  rw [M.mk_dest, Equiv.apply_symm_apply]


@[ext 1000]
theorem ext_destE {α : TypeVec n} {x y : M P α} (h : x.destE = y.destE) : x = y := by
  rw [← destE_mkE (v := x), h, destE_mkE]

/- @[ext 0] -/
theorem ext_mkE {α : TypeVec n} {x y : F.app (α ::: M P α)} (h : mkE x = mkE y) : x = y := by
...
def uLift_up : M P α → M (MvPFunctor.uLift.{u, v} P) α.uLift :=
  .corec' (.mpr TypeVec.uLift_append1_ULift_uLift <$$> MvPFunctor.uLift_up ·.down.dest)
    ∘ .up
def uLift_down : M (MvPFunctor.uLift.{u, v} P) α.uLift → M P α :=
  .corec (transliterateMap ULift.up ·.dest)

@[simp]
theorem dest_uLift_up (x : M P α)
    : (uLift_up x).dest
...
@[simp]
theorem dest_uLift_down (x : M (MvPFunctor.uLift.{u, v} P) α.uLift)
    : (uLift_down x).dest
    = MvPFunctor.uLift_down
...
@[simp]
theorem uLift_up_down {x : M P.uLift α.uLift} : uLift_up (uLift_down x) = x := by
  apply M.bisim (fun x y => x = y.uLift_down.uLift_up) rfl
  rintro _ x rfl
...
@[simp]
theorem uLift_down_up {x : M P α} : uLift_down (uLift_up x) = x := by
  apply M.bisim (fun x y => x = y.uLift_up.uLift_down) rfl
  rintro _ x rfl
...
def transpNatIso
  ...
  nat' {_ _ f} x := by
    ...
    apply bisim (fun a b =>
      ∃ y : M _ _, corec' ((f ::: id) <$$> h.equiv ·.dest) y = a 
